 ...
 */
const _dynamicImport = new _Function(
	"modulePath",
...
export class ConfigLoader {
	...
	private async loadConfig(configPath: string, directory: string) {
		try {
			...
			// const reqPreProcess = _Function(configFile);
			let config = this.disabled
				? {}
				: (await this.dynamicImport(configFile))?.default;
...
export const setConfigLoader = (key: keyof typeof configLoader, value: any) => {
