...
export class JsOrTsComponentInfoProvider implements ComponentInfoProvider {
  ...
  getProp(propName: string): ts.CompletionEntry[] {
    ...
    const propDef = this.typeChecker.getTypeOfSymbolAtLocation(
      prop,
    ...
    const types = flatten(
      propDef.types.map((type) => this.getStringLiteralTypes(type))
    );
  ...
  private getStringLiteralTypes(
    ...
    if (type.isUnion()) {
      return flatten(
        type.types.map((t) => this.getStringLiteralTypes(t, uniques))
      );
    }
