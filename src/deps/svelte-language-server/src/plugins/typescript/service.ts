...
async function createLanguageService(
		...
		return Array.from(
			new Set([
				...
				...snapshotManager
					...
					.filter(
						(file) =>
							!canonicalProjectFileNames.has(getCanonicalFileName(file)),
					),
				...svelteTsxFiles,
			]),
		);
	...
	function getParsedConfig() {
		...
		let configJson =
			(tsconfigPath &&
				ts.readConfigFile(tsconfigPath, tsSystem.readFile).config) ||
			getDefaultJsConfig();
	...
	function watchConfigFile() {
		...
		for (const config of extendedConfigPaths) {
			...
			extendedConfigWatchers.set(
				...
				tsSystem.watchFile(
					config,
					createWatchExtendedConfigCallback(docContext),
					1000,
				),
			);
		}
 ...
 */
function createWatchExtendedConfigCallback(
	docContext: LanguageServiceDocumentContext,
) {
	return async (
		fileName: string,
		...
		const promises = Array.from(
			extendedConfigToTsConfigPath.get(fileName) ?? [],
		).map(async (config) => {
