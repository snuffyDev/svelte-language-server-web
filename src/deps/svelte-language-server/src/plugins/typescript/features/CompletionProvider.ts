...
export interface CompletionEntryWithIdentifier
...
export class CompletionsProviderImpl
	implements CompletionsProvider<CompletionEntryWithIdentifier>
{
	constructor(
	...
	async getCompletions(
		...
		const attributeContext =
			componentInfo && getAttributeContextAtPosition(document, position);
		...
		if (basename(filePath).startsWith("+")) {
			...
			for (const $typeImport of $typeImports.values()) {
				...
				const routesFolder =
					document.config?.kit?.files?.routes || "src/routes";
	...
	private toCompletionItem(
		...
		let { label, insertText, isSvelteComp, replacementSpan } =
		...
		const textEdit = replacementSpan
			? TextEdit.replace(
					convertRange(snapshot, replacementSpan),
					insertText ?? label,
			  )
			: undefined;
	...
	private getCompletionDocument(
		...
		let parts =
			compDetail.codeActions?.map((codeAction) => codeAction.description) ?? [];
		...
		if (sourceDisplay && is$typeImport) {
			...
			parts = parts.map((detail) =>
				detail.replace(
					importPath,
					`'./$types${importPath.endsWith(".js") ? ".js" : ""}'`,
				),
			);
		}

		parts.push(
			changeSvelteComponentName(ts.displayPartsToString(displayParts)),
		);
...
function isValidCompletion(
	...
	const isNoSvelte2tsxCompletion = (value: ts.CompletionEntry) => {
		if (value.kindModifiers === "declare") {
			return (
				!value.name.startsWith("__sveltets_") &&
				!svelte2tsxTypes.has(value.name)
			);
		}
	...
	return (value) =>
		isNoSvelte2tsxCompletion(value) && noWrongCompletionAtStartTag(value);
}
