...
export class CodeActionsProviderImpl implements CodeActionsProvider {
	...
	private async createVirtualDocumentForCombinedImportCodeFix(
		...
		const newText = document.scriptInfo
			? text.slice(0, document.scriptInfo.end) +
			  inserts +
	...
	private removeDuplicatedComponentImport(
		...
		for (const name of insertedNames) {
			...
			const matchRegex =
				unSuffixedNames != name && this.toImportMemberRegex(unSuffixedNames);
			if (
				!matchRegex ||
				!change.textChanges.some((textChange) =>
					textChange.newText.match(matchRegex),
				)
			) {
				continue;
			}
	...
	private async applyQuickfix(
		...
		return codeActionsNotFilteredOut
	...
	private getComponentImportQuickFix(
		...
		for (const [name, position] of nameToPosition) {
			...
			const entries = completion?.entries
				.filter(
					(c) =>
						c.name === name || c.name === toGeneratedSvelteComponentName(name),
				)
	...
	private getSvelteQuickFixes(
		...
		for (const diagnostic of cannotFindNameDiagnostics) {
			...
			const isQuickFixTargetTargetStore = identifier?.escapedText
				.toString()
				.startsWith("$");
	...
	private getEventHandlerQuickFixes(
		...
		const useJsDoc =
			tsDoc.scriptKind === ts.ScriptKind.JS ||
			tsDoc.scriptKind === ts.ScriptKind.JSX;
		const parametersText = (
			useJsDoc
				? parameters.map((p) => p.name)
				: parameters.map(
						(p) => p.name + (p.typeString ? ": " + p.typeString : ""),
				  )
		).join(", ");
		...
		const newText = [
			...
			.map(
				(line) => formatCodeBasis.baseIndent + line + formatCodeBasis.newLine,
			)
			.join("");
	...
	private isQuickFixForEventHandler(
		...
		const htmlNode = document.html.findNodeAt(
