...
import { FoldingRangeKind, Range } from "vscode-languageserver/browser";
...
} from "../../../lib/foldingRange/indentFolding";
...
export class FoldingRangeProviderImpl implements FoldingRangeProvider {
  ...
  async getFoldingRanges(document: Document): Promise<FoldingRange[]> {
    ...
    const result = foldingRanges
      .filter(
        (span) => !isTextSpanInGeneratedCode(tsDoc.getFullText(), span.textSpan)
      )
      .map((span) => ({
      ...
      .filter((r) =>
        lineFoldingOnly ? r.startLine < r.endLine : r.startLine <= r.endLine
      );
  ...
  private mapToOriginalRange(
    ...
    const generatedText = tsDoc.getText(
      textSpan.start,
  ...
  private collectSvelteBlockFolding(
    ...
    const enter: SvelteNodeWalker["enter"] = function (node, parent, key) {
      ...
      if (
        ...
        const expressionEnd =
          (node.type === "CatchBlock"
            ? parent.error?.end
            : parent.value?.end) ?? document.getText().indexOf("}", node.start);

        const beforeBlockStartTagEnd = document
          .getText()
          .indexOf("}", expressionEnd);
      ...
      if (isEachBlock(node)) {
        const start = document
          .getText()
          .indexOf("}", (node.key ?? node.expression).end);
  ...
  private convertOutliningSpan(
    ...
    const result = {
      ...
      startCharacter: lineFoldingOnly
        ? undefined
  ...
  private adjustFoldingEndToNotHideEnd(
    ...
    if (range.end.character > 0) {
      ...
      if (this.foldEndPairCharacters.includes(foldEndCharacter)) {
        return {
          line: this.previousLineOfEndLine(range.start.line, range.end.line),
        };
      }
    }
