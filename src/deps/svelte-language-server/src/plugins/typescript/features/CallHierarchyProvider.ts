...
export class CallHierarchyProviderImpl implements CallHierarchyProvider {
  ...
  async prepareCallHierarchy(
    ...
    const result = await Promise.all(
      itemsArray.map((item) =>
        this.convertCallHierarchyItem(snapshots, item, program)
      )
    );
  ...
  private async convertCallHierarchyItem(
    ...
    return {
      ...
      detail,
      tags: item.kindModifiers?.includes("deprecated")
  ...
  private getNameAndDetailForItem(
    ...
    const nearestRoot =
      nearestRootUri && (urlToPath(nearestRootUri) ?? undefined);
  ...
  async getOutgoingCalls(
    ...
    return result
      ...
      .filter((item) => item.fromRanges.length);
  }
      ...
      ?.map(
        (entry) =>
          [
            ...
            entry.references
              .map((ref) =>
                this.getComponentStartTagFromReference(program, ref)
              )
              .filter(isNotNullOrUndefined),
          ] as const
      )
