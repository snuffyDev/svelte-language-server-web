import { Position, WorkspaceEdit, Range } from "vscode-languageserver/browser";
import {
  Document,
  mapRangeToOriginal,
  getLineAtPosition,
...
export class RenameProviderImpl implements RenameProvider {
  ...
  async rename(
    ...
    let convertedRenameLocations: TsRenameLocation[] =
    ...
    convertedRenameLocations.push(
      ...(await this.enhanceRenamesInCaseOf$Store(
        renameLocations,
        newName,
        docs,
        lang
  ...
  private getRenameInfo(
    ...
    if (tsDoc.getFullText().charAt(renameInfo.triggerSpan.start) === "$") {
      const definition = lang.getDefinitionAndBoundSpan(
        tsDoc.filePath,
        generatedOffset
      )?.definitions?.[0];
      if (
        definition &&
        isTextSpanInGeneratedCode(tsDoc.getFullText(), definition.textSpan)
      ) {
  ...
  private async getAdditionalLocationsForRenameOfPropInsideOtherComponent(
    ...
    const doc = <SvelteDocumentSnapshot>(
      snapshots.get(updatePropLocation.fileName)
    );
  ...
  private isInSvelte2TsxPropLine(
    ...
    return isAfterSvelte2TsxPropsReturn(
      snapshot.getFullText(),
      loc.textSpan.start
    );
  }
  ...
  private async mapAndFilterRenameLocations(
    ...
    return this.filterWrongRenameLocations(
      mappedLocations.filter(isNotNullOrUndefined)
    );
  }
