import { Hover, Position } from "vscode-languageserver/browser";
...
export function getHoverInfo(
  ...
  const attributeOffset = svelteDoc
    .getText()
    .lastIndexOf(attributeContext.name, offset);
...
const tagRegexp = new RegExp(
  `[\\s\\S]*{\\s*(${flatten(tagPossibilities.map((p) => p.values)).join(
    "|"
  )})(\\s|})`
);
