...
export class SvelteDocument {
	...
	async getCompiled(): Promise<SvelteCompileResult> {
		if (!this.compileResult) {
			this.compileResult = await this.getCompiledWith(
				(
					await this.config
				)?.compilerOptions,
			);
...
export class TranspiledSvelteDocument implements ITranspiledSvelteDocument {
	...
	getOriginalPosition(generatedPosition: Position): Position {
		return (
			this.mapper?.getOriginalPosition(generatedPosition) || generatedPosition
		);
	}
	...
	getGeneratedPosition(originalPosition: Position): Position {
		return (
			this.mapper?.getGeneratedPosition(originalPosition) || originalPosition
		);
	}
}
 ...
 */
export class FallbackTranspiledSvelteDocument
	implements ITranspiledSvelteDocument
{
	static async create(
		document: Document,
...
export class SvelteFragmentMapper implements PositionMapper {
	...
	private static create(
		...
		if (originalTagInfo && transpiledTagInfo) {
			const sourceLength =
				originalTagInfo.container.end - originalTagInfo.container.start;
