...
import { FindReferencesProviderImpl as FindFileReferencesProviderImpl } from "./features/FindFileReferencesProvider";
		...
		UpdateTsOrJsFile
{
	...
	private readonly findReferencesProvider: FindFileReferencesProviderImpl;
	...
	constructor(
		...
		this.findReferencesProvider = new FindFileReferencesProviderImpl(
			this.lsAndTsDocResolver,
		);
	...
	async getDefinitions(
		...
		const result = await Promise.all(
			defs.definitions.map(async (def) => {
				const snapshot = await docs.retrieve(def.fileName);
