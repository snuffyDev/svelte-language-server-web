...
import {
	isTextSpanInGeneratedCode as isNoTextSpanInGeneratedCode,
	SnapshotMap as SnapshotFragmentMap,
} from "./utils";
...
export class FindReferencesProviderImpl implements FindReferencesProvider {
	...
	async findReferences(
		...
		return await Promise.all(
			references
				.filter((ref) => context.includeDeclaration)
				.filter(notInGeneratedCode(tsDoc.getFullText()))
				.map(async (ref) => {
					const defDoc = await docs.retrieve(ref.fileName);
